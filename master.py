...
class Application:
    ...
    def HomePage(self, root):
        ...
        # Heading & Text Labels
        MainHeadingLabel = tk.Label(
            root,
            text="Graph Analysis & Simulator",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 30, "bold", "underline"),
        ).pack(pady=20)
        DescriptionHeadingLabel = tk.Label(
            root,
            text="Project-Description: ",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 18, "bold", "underline"),
            anchor="w",
        ).pack(fill="both")
        DescriptionTextLabel = tk.Label(
            root,
            text="'Graphs are a widely used model to describe structural relations. They are built of nodes, which are connected by edges (both directed or undirected). This Graph Simulator simulates different graph algorithms including Maximum Spanning Tree Algorithm, Shortest Path Algorithm and Local Node Clustering Algorithms on a benchmark having graphs with increasing no of nodes. The simulator also provides graphing utility to visualize input and resultant graphs.'",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 14, "bold"),
            wraplengt=1000,
        ).pack(pady=20)
        AlgorithmsHeadingLabel = tk.Label(
            root,
            text="Algorithms:",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 16, "bold", "underline"),
            anchor="w",
        ).pack(fill="both")
        AlgorithmsTextLabel = tk.Label(
            root,
            text="The Algorithms that could be used in analysis of graph are:",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 12, "bold"),
            anchor="w",
        ...
        PrimsLabel = tk.Label(
            ...
            fg=fgColor,
            cursor="hand2",
            font=("TimesNew Roman bold", 12),
        )
        PrimsLabel.pack()
        PrimsLabel.bind(
            "<Button-1>",
            lambda e: self.callback("https://en.wikipedia.org/wiki/Prim%27s_algorithm"),
        )
        # 2
        KruskalLabel = tk.Label(
            root,
        ...
        # 4
        BellmanFordLabel = tk.Label(
            root,
            text="• BellmanFord's Algorithm",
            bg=bgColor,
            fg=fgColor,
            cursor="hand2",
            font=("TimesNew Roman bold", 12),
        ...
        # 5
        FloydWarshallLabel = tk.Label(
            root,
            text="• FloydWarshall's Algorithm",
            bg=bgColor,
            fg=fgColor,
            cursor="hand2",
            font=("TimesNew Roman bold", 12),
        ...
        # 6
        ClusteringCoefficientLabel = tk.Label(
            root,
            text="• Clustering Coefficient Algorithm",
            bg=bgColor,
            fg=fgColor,
            cursor="hand2",
            font=("TimesNew Roman bold", 12),
        ...
        ClusteringCoefficientLabel.bind(
            "<Button-1>",
            lambda e: self.callback(
        ...
        # 7
        BorůvkaLabel = tk.Label(
            root,
            text="• Borůvka's Algorithm",
            bg=bgColor,
            fg=fgColor,
            cursor="hand2",
            font=("TimesNew Roman bold", 12),
        ...
        ProceedToGraphButton = tk.Button(
            root,
            text="Illustrate Graphs",
            bd="8",
            font=("TimesNew Roman bold", 12, "bold"),
            fg=fgColor,
            command=self.MainPage,
        ).place(x=620, y=600)
        ProceedToAlgorithmsButton = tk.Button(
            root,
            text="Analyze Algorithms",
            bd="8",
            font=("TimesNew Roman bold", 12, "bold"),
            fg=fgColor,
            command=self.AlgorithmPage,
        ).place(x=820, y=600)
    ...
    def MainPage(self):
        ...
        MainHeadingLabel = tk.Label(
            ...
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 28, "bold", "underline"),
        ).pack(pady=20)
        NoteLabel = tk.Label(
            self.MainWindow,
            text="Note: ",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 15, "bold", "underline"),
            anchor="w",
        ).pack(fill="both")
        NoteTextLabel = tk.Label(
            self.MainWindow,
            text="You can select number of nodes to be illustrated on a graph from many options given below..",
            bg=bgColor,
            fg="White",
            font=("TimesNew Roman bold", 12),
            wraplengt=1000,
        ...
        InputLabel = tk.Label(
            self.MainWindow,
            text="Select no of Nodes: ",
            bg=bgColor,
            fg=fgColor,
            font=("TimesNew Roman bold", 12),
        ).place(x=400, y=178)
        self.genreCombo = ttk.Combobox(
            self.MainWindow, width=25, values=list(NoOfNodes), state="readonly"
        )
    ...
    def AlgorithmPage(self):
        ...
        NoteTextLabel = tk.Label(
            self.AlgorithmWindow,
            text="You can select number of nodes to be illustrated and select any algorithm from many options given below..",
            bg=bgColor,
            fg="White",
            font=("TimesNew Roman bold", 12),
            wraplengt=1000,
        ...
        self.genreComboNodes = ttk.Combobox(
            self.AlgorithmWindow, width=22, values=list(NoOfNodes)
        )
        self.genreComboNodes.set("10")
        self.genreComboNodes.place(x=400, y=180)
        # ComboBox2
        Inputlabel1 = tk.Label(
            self.AlgorithmWindow,
        ...
        self.genreComboAlgo = ttk.Combobox(
            self.AlgorithmWindow, width=22, values=list(Algorithms)
        )
    ...
    def selectNodes(self):
        ...
        self.srcGenreCombo = ttk.Combobox(
            self.AlgorithmWindow, width=22, values=list(self.SRC_Nodes)
        )
        ...
        self.destGenreCombo = ttk.Combobox(
            self.AlgorithmWindow, width=22, values=list(self.DEST_Nodes)
        )
        ...
        if Algo == Algorithms[6] or Algo == Algorithms[5]:
            ...
            self.destGenreCombo.config(state="disabled")
            self.ViewGraphButton = tk.Button(
                self.AlgorithmWindow,
                text="View Graph",
                bd="8",
                font=("TimesNew Roman bold", 12, "bold"),
                fg=fgColor,
                command=lambda: [self.ViewAlgoGraph(1)],
                state="disabled",
            ).place(x=600, y=450)
        else:
            self.ViewGraphButton = tk.Button(
        ...
        self.ViewResultButton = tk.Button(
            self.AlgorithmWindow,
            text="View Result",
            bd="8",
            font=("TimesNew Roman bold", 12, "bold"),
            fg=fgColor,
            command=lambda: [self.ViewAlgoGraph(0)],
        ).place(x=450, y=450)
    ...
    def ViewAlgoGraph(self, Pos):
        ...
        if Algo == Algorithms[3]:
            ...
            if Pos == 0:
                if dest != "ALL":
                    ...
                    res = "Cost from {} to {} = {}".format(
                        D.src, d, round(D.cost[d], 2)
                    )
                else:
                    ...
                    my_game = ttk.Treeview(
                        game_frame, xscrollcommand=h.set, yscrollcommand=v.set
                    )
        ...
        if Algo == Algorithms[5]:
            ...
            if Pos == 0:
                ...
                for i in range(G.V):
                    my_game.heading(
                        "{}".format(i + 1), text="{}".format(i), anchor=CENTER
                    )
        ...
        if Algo == Algorithms[6]:
            ...
            ws = Tk()
            ws.state("zoomed")
            ws.title("Clustering Coefficient Result")
            ws["bg"] = "#AC99F2"
            game_frame = ttk.Frame(ws)
            game_frame.pack(fill="both", expand=True)
            # scrollbar
            ...
            my_game["columns"] = ("Sno_Nodes", "Local Clustering Cost")

            # format our column
            my_game.column("#0", width=0, stretch=NO)
            my_game.column("Sno_Nodes", anchor=CENTER, width=80)
            my_game.column("Local Clustering Cost", anchor=CENTER, width=80)

            # Create Headings
            my_game.heading("#0", text="Id", anchor=CENTER)
            my_game.heading("Sno_Nodes", text="Nodes", anchor=CENTER)
            my_game.heading(
                "Local Clustering Cost", text="Local Clustering Cost", anchor=CENTER
            )

            for i in range(G.V):
                res = format(L.cluster[i], ".2f")
                my_game.insert(
                    parent="", index="end", iid=i - 1, text="", values=([i, res])
                )
            ...
            my_game.insert(
                ...
                iid=i,
                text="",
                values=(["Average Clustering Cost", res]),
            )
        ...
        if (
            Algo == Algorithms[0]
            or Algo == Algorithms[1]
            or Algo == Algorithms[2]
            or Algo == Algorithms[3]
            or Algo == Algorithms[4]
        ):
            if Algo == Algorithms[0]:
                obj = P
                index = 0
        ...
        self.resetBtn = tk.Button(
            self.AlgorithmWindow,
            text="Select Next Algo",
            bd="8",
            font=("TimesNew Roman bold", 12, "bold"),
            fg=fgColor,
            command=lambda: [self.resetStates(Algo)],
        ).place(x=750, y=450)
        # New Window
